...
@section('title', 'تعديل بيانات الجواز - جوازات اليمن')
@section('header_title', 'تعديل طلب رقم: ' . $application->tracking_number)
        ...
        <div class="form-grid">
            ...
            <div class="form-card">
                <div style="display: flex; align-items: center; gap: 12px; margin-block-end: 24px; padding-block-end: 16px; border-block-end: 1px solid #F3F4F6;">
                    ...
                    <h3 style="margin: 0; font-size: 1.1rem; color: var(--primary);">تعديل البيانات الشخصية</h3>
                </div>

                <div class="form-group" style="margin-block-end: 20px;">
                    <label for="full_name">الاسم الكامل (كما هو في البطاقة الشخصية)</label>
                    <input type="text" name="full_name" id="full_name" class="form-input" required value="{{ old('full_name', $application->full_name) }}">
                </div>

                <div class="form-group" style="margin-block-end: 20px;">
                    <label for="mother_name">اسم الأم</label>
                    <input type="text" name="mother_name" id="mother_name" class="form-input" value="{{ old('mother_name', $application->mother_name) }}">
                </div>

                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-block-end: 20px;">
                    <div class="form-group">
                        <label for="date_of_birth">تاريخ الميلاد</label>
                        <input type="date" name="date_of_birth" id="date_of_birth" class="form-input" required value="{{ old('date_of_birth', $application->date_of_birth?->format('Y-m-d')) }}">
                    </div>
                    <div class="form-group">
                ...
                <div class="form-group" style="margin-block-end: 20px;">
                    ...
                    <input type="text" name="national_id" id="national_id" class="form-input" required value="{{ old('national_id', $application->national_id) }}">
                </div>

                <div class="form-group">
                    <label for="address">العنوان</label>
                    <textarea name="address" id="address" class="form-input" style="block-size: 100px; padding-block: 12px;">{{ old('address', $application->address) }}</textarea>
                </div>
            </div>
            ...
            <div style="display: flex; flex-direction: column; gap: var(--space-lg);">
                    ...
                    <div class="photo-upload-box" onclick="document.getElementById('photo').click()">
                        ...
                        <div id="photo-preview-container">
                            @if($application->photo_path)
                                <img src="{{ asset('storage/' . $application->photo_path) }}" style="inline-size: 120px; block-size: 160px; object-fit: cover; border-radius: 8px; margin-inline: auto; margin-block-end: 12px; border: 4px solid white; box-shadow: var(--shadow-md);">
                                <p style="margin: 0; font-size: 0.85rem; color: var(--accent); font-weight: 700;">تغيير الصورة</p>
                            @else
                ...
                <div class="form-card" style="padding: 24px;">
                    ...
                    <div class="form-group">
                        ...
                        <select name="pickup_branch_id" id="pickup_branch_id" class="form-input" required>
                            @foreach($branches as $branch)
                                <option value="{{ $branch->id }}" {{ $application->pickup_branch_id == $branch->id ? 'selected' : '' }}>{{ $branch->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div style="margin-block-start: auto;">
                    <button type="submit" class="btn-figma-gold" style="inline-size: 100%; block-size: 64px;">
                        حفظ التعديلات
                    </button>
                    <a href="{{ route('admin.applications.show', $application) }}" style="display: block; text-align: center; margin-block-start: 16px; color: var(--text-muted); text-decoration: none; font-size: 0.9rem;">إلغاء والعودة للتفاصيل</a>
                </div>
            </div>
        </div>
    </form>
...
<script>
        ...
        if (input.files && input.files[0]) {
            ...
            reader.onload = function(e) {
                ...
                container.innerHTML = `
                    ...
                    <p style="margin: 0; font-size: 0.85rem; color: var(--accent); font-weight: 700;">صورة جديدة مختارة</p>
                    <button type="button" onclick="resetImage()" style="background: none; border: none; color: #EF4444; font-size: 0.75rem; cursor: pointer; margin-block-start: 4px;">تراجع</button>
                `;
            }
    ...
    function resetImage() {
        location.reload(); // Simplest way to reset to original state
    }
</script>
@endsection
